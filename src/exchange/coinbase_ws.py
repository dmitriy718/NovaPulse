...
class CoinbaseWebSocketClient:
    ...
    async def _subscribe(
        ...
        existing_sub = self._subscriptions.get(channel, {"product_ids": [], "params": {}})
        merged_product_ids = list(
            dict.fromkeys([*existing_sub.get("product_ids", []), *product_ids])
        )
        merged_params = dict(existing_sub.get("params", {}))
        if params:
    ...
    async def _resubscribe(self) -> None:
        ...
        for channel, sub in self._subscriptions.items():
            product_ids = list(dict.fromkeys(sub.get("product_ids", [])))
