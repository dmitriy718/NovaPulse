"""
Secure Vault - Encrypted storage for API keys and secrets.

Encrypts sensitive credentials at rest using the cryptography library's
Fernet scheme (AES-128-CBC + HMAC for authenticated encryption), with a
PBKDF2-derived key from a user password.
...
class SecureVault:
    """
    Password-encrypted vault for API keys and sensitive data.
