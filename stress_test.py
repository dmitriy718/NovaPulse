...
class StressTester:
    ...
    async def _test_api_health(self):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                for ep in endpoints:
                    try:
                        r = await client.get(f"{self.DASHBOARD_URL}{ep}", headers=headers)
    ...
    async def _test_data_freshness(self):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                headers = {"X-API-Key": self.api_key} if self.api_key else {}
                r = await client.get(f"{self.DASHBOARD_URL}/api/v1/scanner", headers=headers)
                scanner = r.json()
    ...
    async def _test_trading_activity(self):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                headers = {"X-API-Key": self.api_key} if self.api_key else {}
                r = await client.get(f"{self.DASHBOARD_URL}/api/v1/status", headers=headers)
                status = r.json()
                scan = status.get("scan_count", 0)
                ws = status.get("ws_connected", False)

                r2 = await client.get(f"{self.DASHBOARD_URL}/api/v1/performance", headers=headers)
                perf = r2.json()
